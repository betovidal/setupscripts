...
_complete_symfony() {
    ...
    for (( i=1; i <= COMP_CWORD; i++ )); do
        if [[ "${COMP_WORDS[i]}" != -* ]]; then
            case "${COMP_WORDS[i]}" in
                php|pecl|pear|php-fpm|php-cgi|php-config|phpdbg|phpize|local:run|run)
                    _command_offset $i
                    return
                    ;;
            esac;
    ...
    local sfcomplete
    if sfcomplete=$(COMP_LINE="${COMP_LINE}" COMP_POINT="${COMP_POINT}" COMP_DEBUG="$COMP_DEBUG" CURRENT="$cword" /usr/bin/symfony self:autocomplete 2>&1); then
